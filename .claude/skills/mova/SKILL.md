...
1. **感知（Sense）**
   ...
   - 必要に応じて POST /mic/record で環境音を録音する（録音前にモーターを停止すること）
     - rms で周囲の音量を判断できる（静か < 0.01、会話 0.01-0.1、騒音 > 0.1）
...
| フィールド | 型 | 値 |
|---|---|---|
| duration | float | 0.5〜3.0（秒）。省略時は1.0 |
| sample_rate | int | 16000（現在この値のみ対応） |
...
```json
{
  ...
  "duration": 2.0,
  "sample_rate": 16000,
  "audio": "<Base64 PCM int16 mono>"
}
```

| フィールド | 説明 |
|---|---|
| rms | RMS音量（0.0〜1.0 正規化。静か < 0.01、会話 0.01-0.1、騒音 > 0.1） |
| peak | ピーク絶対値（int16範囲: 0-32768） |
| audio | Base64エンコードPCM（int16, mono）。将来的にSTT連携等に利用可能 |
...
| 500 | メモリ不足 |
| 503 | 録音中（/mic/record の同時リクエスト不可）、またはマイク未初期化 |
